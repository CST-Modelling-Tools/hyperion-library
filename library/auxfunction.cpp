...
double hypl::auxfunction::DniDailyEnergy(double declination, const Location& location, const Atmosphere& atmosphere)
{
    double wo = location.HourAngleLimit(declination);
    ...
    DniASHRAE dni(declination, location, atmosphere);
    return 2.0 * DEIntegrator<DniASHRAE>::Integrate(dni, t_start, t_end, 1e-6);
}

double hypl::auxfunction::DniYearlyEnergy(const Location& location, const Atmosphere& atmosphere, int delta_days = 26)
{
    double sum = 0.0;
    int day_index = 1;
    while (day_index < 183)
    {
        sum += DniDailyEnergy(SolarDeclinationByIndex(day_index), location, atmosphere);
        day_index += delta_days;
    }
    return (2.0*sum)+DniDailyEnergy(SolarDeclinationByIndex(183), location, atmosphere); // LOOK
}
